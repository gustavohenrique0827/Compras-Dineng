...
import { Users, Plus, Search, MapPin, Phone, Mail, Building, X, ArrowRight } from 'lucide-react';
...
const Suppliers = () => {
  ...
  const filteredSuppliers = suppliers.filter((supplier: Supplier) => {
    ...
    return safeStringIncludes(supplier.nome, searchTerm) || safeStringIncludes(supplier.categoria, searchTerm) || safeStringIncludes(supplier.contato, searchTerm) || safeStringIncludes(supplier.cnpj, searchTerm);
  });
  const form = useForm({
    defaultValues: {
      nome: "",
            ...
            <Card className="glass-card animate-fadeIn">
              <CardHeader>
                ...
                <div className="mt-4 relative">
                  ...
                  <Input placeholder="Pesquisar fornecedores..." className="pl-9" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                </div>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {isLoading ? <div className="col-span-2 text-center py-8">
                      <p className="text-muted-foreground">Carregando fornecedores...</p>
                    </div> : filteredSuppliers.length === 0 ? <div className="col-span-2 text-center py-8">
                      <p className="text-muted-foreground">Nenhum fornecedor encontrado</p>
                    </div> : filteredSuppliers.map((supplier: Supplier) => <div key={supplier.id} className="border rounded-lg p-4 hover:border-primary/30 transition-all cursor-pointer" onClick={() => handleSupplierClick(supplier)}>
                        <div className="flex justify-between items-start">
                          <h3 className="font-medium text-lg">{supplier.nome || 'Sem nome'}</h3>
                          <Button variant="ghost" size="sm" className="text-primary p-0" onClick={e => {
                      e.stopPropagation();
          ...
          {selectedSupplier && <>
              <DialogHeader>
                <div className="flex items-start justify-between">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => setOpenDetailsDialog(false)} className="h-6 w-6">
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {(selectedSupplier.cidade || selectedSupplier.estado) && <div className="ml-6">
                        <span>
                          {[selectedSupplier.cidade, selectedSupplier.estado].filter(Boolean).join(' - ')}
                        </span>
                      </div>}
                    {selectedSupplier.cep && <div className="ml-6">
              ...
              {selectedSupplier.observacoes && <div className="pt-2 border-t">
